...
from tvm.lang.model import (W_Root, w_boolean, W_Integer, symbol, W_Symbol,
                            w_unspec, W_Unspecified, W_Boolean)
...
@elidable
def wtype_to_lltype(w_type):
    for w_sometype, w_lltype in [(W_Unspecified, lltype.Void),
                                 (W_Integer, rffi.LONG),
                                 (W_Boolean, rffi.INT),
                                 (W_Symbol, rffi.CCHARP)]:
        if w_type is w_sometype:
            return w_lltype
    assert 0
...
class W_CDLL(W_Root):
    ...
    def getpointer(self, name, argtypes_w, w_restype):
        ...
        return W_ForeignFunction(funcptr, name, argtypes_w, w_restype)

    def to_string(self):
        return '#<CDLL %s>' % self.libpath

class W_ForeignFunction(W_NativeFunction):
    _immutable_ = True
    ...
    def to_string(self):
        return '#<ForeignFunction %s>' % self.name
    ...
    def call(self, args_w):
        ...
        for i in xrange(len(args_w)):
            ...
            if isinstance(w_arg, W_Integer):
                ...
                charp = rffi.str2charp(w_arg.sval)
        ...
        if self.w_restype is W_Integer:
            ...
            w_retval = w_boolean(rffi.cast(rffi.INT, res))
